"""
Treasury Service - Creates movements in cont_movimiento_tesoreria.
This is the SINGLE source of truth for all real cash movements.
Every collection (cobranza) or payment (pago) must create a treasury movement.
"""
import logging
...
async def create_movimiento_tesoreria(
    ...
    """Insert a treasury movement and return its id."""
    ...
    """, empresa_id, fecha, tipo, monto, cuenta_financiera_id, forma_pago,
        referencia, concepto, origen_tipo, origen_id,
        marca_id, linea_negocio_id, centro_costo_id, proyecto_id, notas)
    logger.info(f"Treasury movement created: id={row['id']} tipo={tipo} monto={monto} origen={origen_tipo}:{origen_id}")
    return row['id']


async def delete_movimientos_by_origen(conn, empresa_id: int, origen_tipo: str, origen_id: int):
    """Delete all treasury movements linked to a specific origin (for reversals)."""
    ...
    logger.info(f"Treasury movements deleted for origen={origen_tipo}:{origen_id}")
    return deleted
