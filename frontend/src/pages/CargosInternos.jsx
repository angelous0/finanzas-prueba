...
export default function CargosInternos() {
  ...
  const s = {
    ...
    select: { padding: '8px 12px', borderRadius: 6, border: '1px solid #d1d5db', fontSize: '0.85rem', background: '#fff' },
    badge: (color) => ({
      ...
      {cargos.length > 0 && (
        <div style={s.summaryRow}>
          <div style={s.summaryCard('#f0fdf4')}>
            <div style={{ fontSize: '0.7rem', fontWeight: 600, color: '#15803d', textTransform: 'uppercase' }}>Total Importe</div>
            <div style={{ fontSize: '1.1rem', fontWeight: 800, color: '#15803d', fontFamily: "'JetBrains Mono', monospace" }}>{formatCurrency(totalImporte)}</div>
          </div>
          ...
          <div style={s.summaryCard('#f8fafc')}>
            <div style={{ fontSize: '0.7rem', fontWeight: 600, color: '#64748b', textTransform: 'uppercase' }}>Cargos</div>
            <div style={{ fontSize: '1.1rem', fontWeight: 800, color: '#334155' }}>{cargos.length}</div>
          </div>
        </div>
      )}
