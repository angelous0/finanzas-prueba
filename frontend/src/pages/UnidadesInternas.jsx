...
export default function UnidadesInternas() {
  ...
  const s = {
    ...
    formCard: { background: '#fff', borderRadius: 12, padding: '1.5rem', width: 420, boxShadow: '0 10px 40px rgba(0,0,0,0.15)' },
    ...
    badge: (color) => ({
      ...
      color: color === 'green' ? '#15803d' : color === 'blue' ? '#1d4ed8' : '#64748b'
    }),
  };
              ...
              {unidades.map(u => {
                ...
                return (
                  <tr key={u.id} data-testid={`unidad-row-${u.id}`}>
                    <td style={{ ...s.td, fontWeight: 600, color: '#0f172a' }}>{u.nombre}</td>
                    ...
                    <td style={s.td}>{personasUnit.length > 0 ? personasUnit.map(p => p.nombre).join(', ') : <span style={{ color: '#94a3b8' }}>Sin personas</span>}</td>
      ...
      {tab === 'personas' && (
        <div style={s.card}>
          ...
          <table style={s.table}>
            ...
            <tbody>
              {personas.length === 0 && (
                <tr><td colSpan={4} style={{ ...s.td, textAlign: 'center', color: '#94a3b8', padding: 32 }}>
        ...
        <div style={s.formOverlay} onClick={() => setShowForm(false)}>
          <div style={s.formCard} onClick={e => e.stopPropagation()} data-testid="unidad-form-modal">
            ...
            <div style={{ marginBottom: '1rem' }}>
              <label style={{ fontSize: '0.75rem', fontWeight: 600, color: '#64748b', display: 'block', marginBottom: 4 }}>Tipo</label>
