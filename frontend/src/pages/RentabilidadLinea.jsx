  ...
  return (
    <div style={{ padding: '1.5rem', maxWidth: '1400px' }} data-testid="rentabilidad-linea-page">
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.25rem' }}>
        <div>
          <h1 style={{ fontSize: '1.25rem', fontWeight: 700, margin: 0, color: '#0f172a' }}>Rentabilidad x Linea</h1>
          <p style={{ fontSize: '0.8rem', color: '#64748b', margin: '0.25rem 0 0' }}>Control de dinero por linea de negocio</p>
        </div>
    ...
    <div data-testid="cruce-linea-marca">
      {data.map((linea) => (
        <div key={linea.linea} style={{ background: '#fff', border: '1px solid #e2e8f0', borderRadius: '8px', marginBottom: '0.75rem', overflow: 'hidden' }}>
          <div style={{ padding: '0.625rem 1rem', borderBottom: '1px solid #f1f5f9', display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: '#f8fafc' }}>
            <span style={{ fontWeight: 700, fontSize: '0.875rem', color: '#0f172a' }}>{linea.linea}</span>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.8rem' }} data-testid={`cruce-table-${linea.linea}`}>
            <thead>
              <tr>
                <th style={{ padding: '4px 12px', textAlign: 'left', color: '#64748b', fontWeight: 500, fontSize: '0.75rem' }}>Marca</th>
                <th style={{ padding: '4px 12px', textAlign: 'right', color: '#64748b', fontWeight: 500, fontSize: '0.75rem' }}>Ventas</th>
                <th style={{ padding: '4px 12px', textAlign: 'right', color: '#64748b', fontWeight: 500, fontSize: '0.75rem' }}>Tickets</th>
                <th style={{ padding: '4px 12px', textAlign: 'right', color: '#64748b', fontWeight: 500, fontSize: '0.75rem' }}>%</th>
              </tr>
            </thead>
            <tbody>
              {linea.marcas.map((m) => (
                <tr key={m.marca} style={{ borderTop: '1px solid #f1f5f9' }}>
                  ...
                  <td style={{ padding: '5px 12px', textAlign: 'right' }}>
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-end', gap: '0.5rem' }}>
                      ...
                      <span style={{ fontSize: '0.75rem', color: '#64748b', minWidth: '36px', textAlign: 'right' }}>{pct(m.pct)}</span>
                    </div>
                  </td>
                </tr>
              ))}
...
function KPI({ label, value, color, bold }) {
  return (
    <div style={{ background: '#fff', border: '1px solid #e2e8f0', borderRadius: '8px', padding: '0.75rem 1rem' }}>
      <div style={{ fontSize: '0.7rem', color: '#64748b', textTransform: 'uppercase', letterSpacing: '0.5px', marginBottom: '0.25rem' }}>{label}</div>
      ...
      <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.8125rem' }} data-testid={testId}>
        <thead>
          <tr style={{ background: '#f8fafc' }}>
            {headers.map((h, i) => (
              <th key={i} style={{ padding: '0.5rem 0.75rem', textAlign: i === 0 ? 'left' : 'right', color: '#475569', fontWeight: 600, borderBottom: '2px solid #e2e8f0', fontSize: '0.75rem' }}>{h}</th>
            ))}
          </tr>
        </thead>
        <tbody>
