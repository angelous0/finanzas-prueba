      ...
      <div style={{ display: 'grid', gridTemplateColumns: '280px 1fr', gap: '1rem', marginBottom: '1rem' }}>
        <Card title="Ventas Pendientes" icon={BarChart3} testId="rep-ventas-pend">
          <div style={{ textAlign: 'center', padding: '0.75rem 0' }}>
            ...
            <div style={{ fontSize: '0.8rem', color: '#64748b' }}>por revisar</div>
...
function ReportTable({ headers, rows, testId }) {
  if (!rows.length) return <p style={{ textAlign: 'center', color: '#94a3b8', fontSize: '0.8rem', padding: '1rem' }}>Sin datos</p>;
  return (
    <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.8rem' }} data-testid={testId}>
      <thead>
        <tr>
          {headers.map((h, i) => (
            <th key={i} style={{ padding: '4px 8px', textAlign: i === 0 ? 'left' : 'right', color: '#64748b', fontWeight: 500, borderBottom: '1px solid #e2e8f0', fontSize: '0.75rem' }}>{h}</th>
          ))}
        </tr>
      </thead>
      <tbody>
