...
const CAT_COLORS = {
  ...
  'Rigido': { bg: '#f0fdf4', text: '#166534', border: '#86efac' },
  'PT': { bg: '#f0f9ff', text: '#0369a1', border: '#7dd3fc' },
};

const getCatColor = (cat) => CAT_COLORS[cat] || { bg: '#f8fafc', text: '#64748b', border: '#e2e8f0' };
...
export default function ValorizacionInventario() {
  ...
  if (!eId) return <div style={{ padding: '2rem', color: '#94a3b8' }}>Seleccione una empresa</div>;
  ...
  const s = {
    ...
    subtitle: { fontSize: '0.8rem', color: '#64748b', marginTop: 2 },
    kpiRow: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 12, marginBottom: '1rem' },
    kpiCard: (borderColor) => ({
      background: '#fff', borderRadius: 10, padding: '0.85rem 1rem', border: '1px solid #e2e8f0',
      borderLeft: `4px solid ${borderColor}`
    }),
    kpiLabel: { fontSize: '0.7rem', fontWeight: 600, color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '0.5px' },
    ...
    filterBar: {
      ...
      background: '#fff', borderRadius: 10, padding: '0.75rem 1rem', border: '1px solid #e2e8f0'
    },
    input: { padding: '7px 12px', borderRadius: 6, border: '1px solid #d1d5db', fontSize: '0.8rem', minWidth: 180 },
    select: { padding: '7px 12px', borderRadius: 6, border: '1px solid #d1d5db', fontSize: '0.8rem', background: '#fff' },
    btn: { padding: '6px 12px', borderRadius: 6, border: '1px solid #e2e8f0', cursor: 'pointer', fontSize: '0.75rem', fontWeight: 600, display: 'inline-flex', alignItems: 'center', gap: 6, background: '#fff', color: '#64748b' },
    btnActive: { background: '#f0fdf4', color: '#16a34a', borderColor: '#bbf7d0' },
    catHeader: (color) => ({
    ...
    card: { background: '#fff', borderRadius: 10, border: '1px solid #e2e8f0', overflow: 'hidden', marginBottom: '0.75rem' },
    table: { width: '100%', borderCollapse: 'collapse', fontSize: '0.78rem' },
    th: { padding: '8px 12px', textAlign: 'left', fontWeight: 600, color: '#94a3b8', borderBottom: '1px solid #e2e8f0', fontSize: '0.7rem', textTransform: 'uppercase' },
    ...
    footerRow: { background: '#f8fafc', fontWeight: 800, borderTop: '2px solid #e2e8f0' },
  };
  ...
  return (
    <div style={s.page} data-testid="valorizacion-page">
      ...
      {data && (
        <div style={s.kpiRow} data-testid="valorizacion-kpis">
          <div style={s.kpiCard('#3b82f6')}>
            ...
            <div style={{ fontSize: '0.7rem', color: '#64748b', marginTop: 2 }}>{totalConStock} con stock</div>
          </div>
      ...
      {/* Grouped by category */}
      {loading ? (
        <div style={{ ...s.card, padding: 40, textAlign: 'center', color: '#94a3b8' }}>Cargando...</div>
      ) : sortedCats.length === 0 ? (
            ...
            <div key={cat} style={s.card} data-testid={`cat-group-${cat}`}>
              ...
              <div style={s.catHeader(color)} onClick={() => toggleCat(cat)}>
                ...
                <div style={s.catStats}>
                  <span style={{ color: '#64748b' }}>Stock: <strong>{fmtQty(totals.stockTotal)}</strong></span>
                    ...
                    {catItems.map(item => (
                      <React.Fragment key={item.id}>
                        ...
                        >
                          ...
                          <td style={{ ...s.td, fontFamily: "'JetBrains Mono', monospace", fontSize: '0.75rem', color: '#64748b' }}>{item.codigo}</td>
                          <td style={{ ...s.td, fontWeight: 600, color: '#0f172a' }}>{item.nombre}</td>
                          <td style={{ ...s.td, color: '#64748b' }}>{item.unidad}</td>
                          <td style={{
                            ...s.td, textAlign: 'right', fontWeight: 700,
                            fontFamily: "'JetBrains Mono', monospace",
                            color: item.stock_actual < 0 ? '#dc2626' : item.stock_actual > 0 ? '#0f172a' : '#cbd5e1'
                          }}>
                          ...
                          <tr>
                            <td colSpan={9} style={{ padding: 0 }}>
                              <div style={s.lotePanel}>
                                <div style={{ fontSize: '0.7rem', fontWeight: 700, color: '#0369a1', marginBottom: 6, textTransform: 'uppercase' }}>
                                  ...
                                  <tbody>
                                    {item.lotes_fifo.map((lote, i) => (
                                      <tr key={lote.id || i}>
                                        ...
                                        <td style={{ ...s.td, borderColor: '#e0f2fe', textAlign: 'right', fontWeight: 700, fontFamily: "'JetBrains Mono', monospace", color: '#0369a1' }}>
      ...
      {data && sortedCats.length > 0 && (
        <div style={{
          ...
          <div style={{ display: 'flex', gap: 32 }}>
            <div>
              <span style={{ fontSize: '0.65rem', color: '#94a3b8', textTransform: 'uppercase', fontWeight: 600 }}>FIFO</span>
              <div style={{ fontWeight: 800, fontSize: '1.1rem', fontFamily: "'JetBrains Mono', monospace", color: '#4ade80' }}>
                {fmt(data.total_valor_fifo)}
              </div>
            </div>
            <div>
              <span style={{ fontSize: '0.65rem', color: '#94a3b8', textTransform: 'uppercase', fontWeight: 600 }}>Promedio</span>
