...
export default function GastosUnidadInterna() {
  ...
  const s = {
    ...
    formCard: { background: '#fff', borderRadius: 12, padding: '1.5rem', width: 480, boxShadow: '0 10px 40px rgba(0,0,0,0.15)' },
    input: { width: '100%', padding: '8px 12px', borderRadius: 6, border: '1px solid #d1d5db', fontSize: '0.85rem', boxSizing: 'border-box' },
    select: { width: '100%', padding: '8px 12px', borderRadius: 6, border: '1px solid #d1d5db', fontSize: '0.85rem', background: '#fff', boxSizing: 'border-box' },
    badge: (color) => ({
      padding: '2px 8px', borderRadius: 4, fontSize: '0.7rem', fontWeight: 700,
      background: color === 'red' ? '#fef2f2' : color === 'amber' ? '#fffbeb' : '#f1f5f9',
      color: color === 'red' ? '#dc2626' : color === 'amber' ? '#d97706' : '#64748b'
    }),
    filterBar: { display: 'flex', gap: 12, alignItems: 'center', marginBottom: '1rem', flexWrap: 'wrap' },
    summaryCard: { background: '#fef2f2', borderRadius: 8, padding: '0.75rem 1rem', display: 'flex', alignItems: 'center', gap: 12, marginBottom: '0.75rem' },
  };
    ...
    <div style={s.page} data-testid="gastos-unidad-page">
      ...
      {gastos.length > 0 && (
        <div style={s.summaryCard}>
          <span style={{ fontWeight: 700, color: '#dc2626', fontSize: '0.85rem' }}>Total Gastos:</span>
          <span style={{ fontWeight: 800, color: '#dc2626', fontSize: '1rem', fontFamily: "'JetBrains Mono', monospace" }}>
            {formatCurrency(totalGastos)}
          </span>
          <span style={{ color: '#94a3b8', fontSize: '0.75rem' }}>({gastos.length} registros)</span>
        </div>
      )}
      ...
      <div style={s.card}>
        <table style={s.table}>
          ...
          <tbody>
            {gastos.length === 0 && (
              <tr><td colSpan={7} style={{ ...s.td, textAlign: 'center', color: '#94a3b8', padding: 32 }}>
            ...
            {gastos.map(g => (
              <tr key={g.id} data-testid={`gasto-row-${g.id}`}>
                ...
                <td style={{ ...s.td, textAlign: 'right', fontWeight: 700, color: '#dc2626', fontFamily: "'JetBrains Mono', monospace" }}>
                  {formatCurrency(g.monto)}
                </td>
                <td style={s.td}>{g.registro_id || g.movimiento_id ? <span style={s.badge('')}>{g.registro_id || g.movimiento_id}</span> : <span style={{ color: '#94a3b8' }}>General</span>}</td>
                <td style={{ ...s.td, textAlign: 'right' }}>
                  <button style={{ ...s.btn, ...s.btnGhost, marginRight: 4 }} onClick={() => startEdit(g)}><Pencil size={14} /></button>
                  <button style={{ ...s.btn, background: '#fef2f2', color: '#dc2626', border: '1px solid #fecaca' }}
        ...
        <div style={s.formOverlay} onClick={() => setShowForm(false)}>
          <div style={s.formCard} onClick={e => e.stopPropagation()} data-testid="gasto-form-modal">
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.75rem' }}>
              <div>
                <label style={{ fontSize: '0.75rem', fontWeight: 600, color: '#64748b', marginBottom: 4, display: 'block' }}>Fecha</label>
              ...
              <div>
                <label style={{ fontSize: '0.75rem', fontWeight: 600, color: '#64748b', marginBottom: 4, display: 'block' }}>Unidad Interna</label>
              ...
              <div>
                <label style={{ fontSize: '0.75rem', fontWeight: 600, color: '#64748b', marginBottom: 4, display: 'block' }}>Tipo de Gasto</label>
              ...
              <div>
                <label style={{ fontSize: '0.75rem', fontWeight: 600, color: '#64748b', marginBottom: 4, display: 'block' }}>Monto</label>
              ...
              <div style={{ gridColumn: '1 / -1' }}>
                <label style={{ fontSize: '0.75rem', fontWeight: 600, color: '#64748b', marginBottom: 4, display: 'block' }}>Descripcion</label>
              ...
              <div>
                <label style={{ fontSize: '0.75rem', fontWeight: 600, color: '#64748b', marginBottom: 4, display: 'block' }}>Registro ID (opcional)</label>
              ...
              <div>
                <label style={{ fontSize: '0.75rem', fontWeight: 600, color: '#64748b', marginBottom: 4, display: 'block' }}>Movimiento ID (opcional)</label>
