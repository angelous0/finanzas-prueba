      ...
      <div className="modal" style={{ maxWidth: '950px', width: '95%', maxHeight: '90vh', display: 'flex', flexDirection: 'column' }} onClick={e => e.stopPropagation()}>
        ...
        <div className="modal-body" style={{ overflowY: 'auto', padding: '1rem 1.5rem' }}>
          ...
          <div style={{ marginBottom: '1.25rem' }}>
            <h4 style={{ fontSize: '0.8125rem', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.03em', color: '#64748b', marginBottom: '0.75rem' }}>
              Lineas inventariables ({lineasConArticulo.length})
            </h4>
            {lineasConArticulo.length === 0 ? (
              <p style={{ color: '#94a3b8', padding: '1rem', textAlign: 'center' }}>No hay lineas con articulo asignado</p>
            ) : (
                    ...
                    <div key={linea.id}>
                      <div
                        ...
                        style={{
                          ...
                          border: `1.5px solid ${isSelected ? '#1B4D3E' : '#e2e8f0'}`,
                          borderRadius: '8px', cursor: 'pointer', transition: 'all 0.15s',
                          background: isSelected ? '#f0fdf4' : '#fff'
                        }}
                      >
                        <Package size={16} style={{ color: isComplete ? '#16a34a' : '#64748b', flexShrink: 0 }} />
                        <div style={{ flex: 1, minWidth: 0 }}>
                          <div style={{ fontWeight: 500, fontSize: '0.875rem' }}>{linea.descripcion || linea.articulo_nombre || '-'}</div>
                          <div style={{ fontSize: '0.75rem', color: '#64748b' }}>{linea.articulo_codigo || linea.articulo_id}</div>
                        </div>
                      ...
                      {vincs.length > 0 && (
                        <div style={{ marginLeft: '2rem', marginTop: '0.25rem', marginBottom: '0.25rem' }}>
                          {vincs.map(v => (
                            <div key={v.id} style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', padding: '0.25rem 0.5rem', fontSize: '0.75rem', color: '#64748b' }}>
                              <span style={{ fontFamily: "'JetBrains Mono', monospace", color: '#1B4D3E', fontWeight: 500 }}>{parseFloat(v.cantidad_aplicada).toFixed(0)} uds</span>
                              <span style={{ color: '#94a3b8' }}>&larr;</span>
                              <span>{v.ingreso_ref || v.ingreso_id?.substring(0, 8)}</span>
                              <span style={{ color: '#94a3b8' }}>({formatDate(v.ingreso_fecha)})</span>
          ...
          {selectedLinea && (
            <div style={{ borderTop: '1px solid #e2e8f0', paddingTop: '1rem' }}>
              ...
              {loading ? (
                ...
                <p style={{ color: '#94a3b8', padding: '1rem', textAlign: 'center' }}>No hay ingresos para este articulo</p>
                          ...
                          <tr key={ing.id} style={{ opacity: disponible <= 0 ? 0.4 : 1 }}>
                            ...
                            <td>
                              <input
                                ...
                                style={{ width: '100%', textAlign: 'center', padding: '0.25rem', fontSize: '0.8125rem', border: '1px solid #e2e8f0', borderRadius: '4px' }}
