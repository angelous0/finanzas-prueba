...
export default function Tesoreria() {
  ...
  const fetchData = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const params = new URLSearchParams({ empresa_id: eId, page, page_size: 50 });
      if (filtroTipo) params.set('tipo', filtroTipo);
      if (filtroOrigen) params.set('origen_tipo', filtroOrigen);
      if (fechaDesde) params.set('fecha_desde', fechaDesde);
      if (fechaHasta) params.set('fecha_hasta', fechaHasta);

      const resParams = new URLSearchParams({ empresa_id: eId });
      if (fechaDesde) resParams.set('fecha_desde', fechaDesde);
      if (fechaHasta) resParams.set('fecha_hasta', fechaHasta);

      const [movRes, resumenRes] = await Promise.all([
        fetch(`${API}/api/tesoreria?${params}`).then(r => r.json()),
        fetch(`${API}/api/tesoreria/resumen?${resParams}`).then(r => r.json()),
      ]);
      setMovimientos(movRes.data || []);
      setTotal(movRes.total || 0);
      setResumen(resumenRes);
    } catch (e) {
      console.error('Error fetching tesoreria:', e);
    } finally {
      setLoading(false);
    }
